...
df = pd.read_csv("data_preparation_accidents/outlier_datasets/crash_drop_outliers.csv")
...
# Fit MinMaxScaler on entire dataset BEFORE splitting
scaler = MinMaxScaler(feature_range=(0, 1), copy=True).fit(df_features)

# Transform and rebuild df (following your methodology: always overwrite df)
scaled_values = scaler.transform(df_features)
df = pd.DataFrame(scaled_values, columns=df_features.columns, index=df.index)
...
# Required columns without primary cause
required_cols = ["weather_condition", "roadway_surface_cond",
                 "lighting_condition", "trafficway_type", "intersection_related_i"]

for col in required_cols:
    if col not in df.columns:
        print(f"WARNING: {col} not found in df. Some features may not be created.")

# Interaction: weather × road surface
if "weather_condition" in df.columns and "roadway_surface_cond" in df.columns:
    df["weather_surface_interaction"] = df["weather_condition"] * df["roadway_surface_cond"]

# Interaction: lighting × hour of day
...
# Interaction: trafficway type × other factors (without primary cause)
if "trafficway_type" in df.columns and "intersection_related_i" in df.columns:
    df["trafficway_intersection_interaction"] = df["trafficway_type"] * df["intersection_related_i"]

# AGGREGATE RISK SCORE (exclude primary cause)
...
if existing_risk_features:
    ...
    print("WARNING: No features found for risk_score calculation.")

# Find features to drop (you used threshold=3 in your example)
vars2drop = select_low_variance_variables(df, max_threshold=0.001, target=target)

print("Variables to drop:", vars2drop)

# Drop them
df = df.drop(columns=vars2drop, errors="ignore")

# Split BEFORE SMOTE
train_df, test_df = train_test_split(df, test_size=0.3, shuffle=True, random_state=42, stratify=df[target])
...
# Save
train_df.to_csv("data_preparation_accidents/train_datasets/traffic_accidents_train_7.csv", index=False)
test_df.to_csv("data_preparation_accidents/test_datasets/traffic_accidents_test_7.csv", index=False)
...
target = "crash_type"
file_tag = "crash"
train: DataFrame = read_csv("data_preparation_accidents/train_datasets/traffic_accidents_train_7.csv")
test: DataFrame = read_csv("data_preparation_accidents/test_datasets/traffic_accidents_test_7.csv")
...
savefig(f"data_preparation_accidents/preparation_images/{file_tag}_eval_7.png")
show()
...
plot_conf_matrix(tstY, pred_NB, "NB Confusion Matrix (%)", f"data_preparation_accidents/preparation_images/{file_tag}_NB_confusion_7.png")
plot_conf_matrix(tstY, pred_KNN, "KNN Confusion Matrix (%)", f"data_preparation_accidents/preparation_images/{file_tag}_KNN_confusion_7.png")