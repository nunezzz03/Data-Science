...
def get_variable_types(df: DataFrame) -> dict[str, list[str]]:
    ...
    """
    variable_types: dict[str, list[str]] = {"date": [], "symbolic": []}
    ...
    variable_types["symbolic"] = [
        c for c in symbolic_cols if c not in variable_types["date"]
    ]
...
def analyse_property_granularity(
    ...
    fig, axs = subplots(
        1, cols, figsize=(cols * 6, HEIGHT), squeeze=False
    )  # Increased width
...
try:
    data_acc: DataFrame = read_csv(
        os.path.join(PROJECT_ROOT, "data", "raw", "traffic_accidents.csv")
    )
    ...
    for v_date in date_cols:
        analyse_date_granularity(
            data_acc, v_date, ["year", "quarter", "month", "day", "hour"]
        )
    ...
    print("❌ Error: traffic_accidents.csv not found in data/raw/ folder.")
...
try:
    ...
    data_flights: DataFrame = read_csv(
        os.path.join(PROJECT_ROOT, "data", "raw", "Combined_Flights_2022.csv")
    ...
    if date_cols:
        ...
        for v_date in date_cols:
            analyse_date_granularity(
                data_flights, v_date, ["year", "quarter", "month", "day"]
            )
    ...
    print(
        f"   ℹ️ Generating symbolic charts for {len(symbolic_vars)} variables (this may take a moment)..."
    )
