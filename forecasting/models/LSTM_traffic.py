...
# Load data
# =========================
filename = Path(__file__).parent / "traffic_smoothed.csv"
file_tag = "TRAFFIC"
target = "Total"
timecol = None  # use default index
...
# =========================
def lstm_study(train, test, nr_episodes: int = 2000, measure: str = "R2"):
    sequence_size = [12, 24, 48]
        ...
        for hidden in nr_hidden_units:
            ...
            for n in range(1, nr_episodes + 1):
                ...
                if n % step == 0 or n == 1:
                    ...
                    print(
                        f"seq={length} hidden={hidden} epochs={n} "
                        f"loss={loss:.5f} {measure}={eval_value:.4f}"
                    )

                    if (
                        eval_value > best_performance
                        and abs(eval_value - best_performance) > DELTA_IMPROVE
                    ):
