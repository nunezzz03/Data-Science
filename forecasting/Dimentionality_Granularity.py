...
def ts_aggregation_by(df: DataFrame | Series, group_size: int, agg_func: str = "mean") -> DataFrame | Series:
    """Aggregate by fixed-size window (used for traffic synthetic timeline)"""
    return df.groupby(df.index // group_size).agg(agg_func)

def ts_aggregation_by_period(df: DataFrame | Series, freq: str, agg_func: str = "mean") -> DataFrame | Series:
...
def plot_ts_multivariate_chart(df: DataFrame, title: str, save_path: Path) -> None:
    ...
    fig: Figure
    axs: list[Axes]
    fig, axs = subplots(len(numeric_cols), 1, figsize=(3*HEIGHT, HEIGHT/2*len(numeric_cols)))
    fig.suptitle(title)
...
for name, freq in econ_grans.items():
    ...
    plot_line_and_save(list(agg.index), agg[econ_target].to_list(),
                       xlabel="Date", ylabel=econ_target,
                       title=f"Economic {name} aggregation ({econ_target})",
                       save_path=fig_path_uni)

    # Multivariate plot
    fig_path_multi = econ_out / f"economic_{name}_multivariate.png"
    plot_ts_multivariate_chart(agg, title=f"Economic {name} multivariate", save_path=fig_path_multi)
